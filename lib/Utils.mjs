...
// 每級成長係數，index = 該軸檔次。
//
// 來源：「噬生・魔物觀測者」monster.exe 的 double 表，VA 0x0058EA14
// （file offset 0x18D614），共 111 筆，index 0..110，最大值 4.615。
//
// 這張表原本只到 52，而且 51/52 是 2.04 / 2.08 —— 從 50 的 2.095 往回掉，
// 明顯不對（原表單調遞增）。正確值是 2.14 / 2.18。
// 檔次 > 52 的寵物過去會查到 undefined，整條計算變成 NaN。
//
// 閉合式（與本表完全一致，保留作為交叉驗證，見 fullRatesFormula）：
//   0.04*n + 0.01*floor((n-1)/5) + (n > 0 && n % 5 === 0 ? 0.005 : 0)
const fullRates = {
    "0": 0.00,
    "1": 0.04,
    "2": 0.08,
...
// fullRates 的閉合式。原程式硬編了整張表，這裡保留公式版本供測試交叉比對，
// 正式查表請一律用 fullRates —— 硬編表不會有浮點漂移。
function fullRatesFormula(n) {
    if (n <= 0) return 0;   // floor((0-1)/5) = -1，不擋會得到 -0.01
    return 0.04 * n
        + 0.01 * Math.floor((n - 1) / 5)
        + (n % 5 === 0 ? 0.005 : 0);
}
...
function GuessResultToString(results, limit, showDetails) {
    ...
    } else if (_results.every(r => r.isApproximate)) {
        out.push(" * 近似解 (BP 為實數，整數隨機檔列舉無精確解，採連續可行性回退；隨機檔分布為四捨五入顯示)")
    }
...
export {sum, calcDiff, loopForSum, _loopForSum, fullRates, fullRatesFormula, minmax, GuessResultToString};