...
class GrowRange {
     ...
     * @param {object} [opts]
     * @param {'exact'|'observer'} [opts.mode='exact']
     *   `exact` 是原本的整數精確比對。
    ...
    guess(stat, notorderpoint = 0, targetGrow = null, opts = {}) {
        ...
        const nudgeTol = mode === 'observer'
            ? observerTolerance(stat.lvl, opts.catchLvl ?? 0)
            : null;

        let res = this.guessWithSpecficLvlPoint(stat, stat.lvl - 1 - notorderpoint, targetGrow, nudgeTol);

        if (res.length == 0 && stat.lvl != 1) {
            res = this.guessWithSpecficLvlPoint(stat, 0, targetGrow, nudgeTol);
        }
