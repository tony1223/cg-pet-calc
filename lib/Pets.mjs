...
class GrowRange {
    ...
    /**
     * @param {Stat} stat 觀測到的能力
     * @param {number} notorderpoint 已知未分配的等級點數
     * @param {GrowRange|null} targetGrow 指定實際檔次（跳過 3125 組列舉）
     * @param {object} [opts]
     * @param {'exact'|'observer'|'auto'} [opts.mode='exact']
     *   `exact` 是原本的整數精確比對。
     *   `observer` 是「噬生・魔物觀測者」的規則：只在檔次落在 fullRates
     *   五週期不規則處（`% 5 ∈ {0, 1}`）時放寬取整邊界。見 tierNudges。
     *   `auto` 由緊到鬆：精確的兩階先跑完，全空了才升級到容差。
     *   要接住「高等野寵算不出來」又不想動到本來就算得出來的案例，用這個。
     * @param {number} [opts.catchLvl=0] 入手等級，用來算容差（0 = 不使用）
     */
    guess(stat, notorderpoint = 0, targetGrow = null, opts = {}) {
        const mode = opts.mode ?? 'exact';
        const point = stat.lvl - 1 - notorderpoint;

        // 依序嘗試 (加點總數, 容差)，第一個有解的就回傳。
        // 跟 Rust 版 solve() 的 attempts 階梯同構 —— 那邊是這套規則的權威移植。
        const rungs = [];
        const ladder = (tol) => {
            rungs.push([point, tol]);
            // 等級 1 沒有等級點數可分配，退到 point = 0 只是把同一次列舉再跑一遍
            if (stat.lvl != 1) rungs.push([0, tol]);
        };
        if (mode === 'auto') {
            ladder(null);
        ...
        for (const [p, tol] of rungs) {
            const res = this.guessWithSpecficLvlPoint(stat, p, targetGrow, tol);
        ...
        if (stat.lvl != 1) {
            return this.guessWithSpecficLvlPointContinuous(stat, point, targetGrow);
        }
    ...
    guessWithSpecficLvlPoint(stat, point, targetGrow, nudgeTol = null) {
        const result = [];
        ...
        if (targetGrow) {
            this.mockLoopRange(this.drop(...targetGrow.toArray()), (sumBP, growRange) => {
                return this._handleGuessingGrowRange(growRange, stat, point, oSum,
                    oUpSum, calcUpBp, results, result, sumBP, nudgeTol);
            })
            return result;
        }
    ...
    _handleGuessingGrowRange(growRange, stat, point, oSum, oUpSum, calcUpBp, results, result, sumBP, nudgeTol = null) {
        if (!growRange.contains(this)) {
            return false;
        }
        const res = growRange.calcBPAtLevel(stat.lvl, point);

        // 觀測者模式：逐軸偏移只跟這組候選檔次有關，建一次給底下所有加點／隨機檔共用。
        // 五軸全都只有 [0] 的話就沒必要走慢路徑 —— 那跟精確比對完全等價。
        const nudges = nudgeTol === null ? null : growRange.toArray().map(t => tierNudges(t, nudgeTol));
        const useNudges = nudges !== null && nudges.some(n => n.length > 1);
        // 底下每一道剪枝都要跟著放寬，否則靠偏移才成立的解根本走不到比對那一步。
        const slack = useNudges ? nudgeSlack(nudges, nudgeTol) : ZERO_SLACK;

        if (!(res.sumFullBP >= oSum - slack.sum && res.sumFullBP <= oUpSum + slack.sum)) return;
            ...
            for (let b = 0; b <= bMax; b++) {
                ...
                for (let c = 0; c <= cMax; c++) {
                    ...
                    for (let d = 0; d <= dMax; d++) {
                        ...
                        if (tHp - 19 - hpBase + sHp <= minHpR || tHp - 20 - hpBase - sHp >= maxHpR) continue;
                        const mpBase = m0 + 2*m1 + 2*m2 + 2*m3 + 10*m4;
                        if (tMp - 19 - mpBase + sMp <= minMpR || tMp - 20 - mpBase - sMp >= maxMpR) continue;
                        const atkBase = 0.2*m0 + 2.7*m1 + 0.3*m2 + 0.3*m3 + 0.2*m4;
                        if (tAtk - 19 - atkBase + sAtk <= minAtkR || tAtk - 20 - atkBase - sAtk >= maxAtkR) continue;
                        const defBase = 0.2*m0 + 0.3*m1 + 3*m2 + 0.3*m3 + 0.2*m4;
                        if (tDef - 19 - defBase + sDef <= minDefR || tDef - 20 - defBase - sDef >= maxDefR) continue;
                        const agiBase = 0.1*m0 + 0.2*m1 + 0.2*m2 + 2*m3 + 0.1*m4;
                        if (tAgi - 19 - agiBase + sAgi <= minAgiR || tAgi - 20 - agiBase - sAgi >= maxAgiR) continue;
                            ...
                            for (let dr1 = -1; dr1 <= 1; dr1++) {
                                ...
                                for (let dr2 = -1; dr2 <= 1; dr2++) {
                                    ...
                                    for (let dr3 = -1; dr3 <= 1; dr3++) {
                                        ...
                                        // 預設走原本的精確比對；觀測者模式且這組檔次真的有
                                        // 不規則軸時，才進慢路徑試偏移量。
                                        let nudged = false;
                                        if (useNudges) {
                                            const verdict = observerMatch(nudges, [x0, x1, x2, x3, x4],
                                                tHp, tMp, tAtk, tDef, tAgi);
                                        ...
                                        result.push({
                                            ...
                                            // 靠取整邊界修補才成立的解 —— 精確模式不會有這個旗標
                                            ...(nudged ? {isNudged: true} : {}),
                                        });
                                    }
                                }
                            }
...
// ── 觀測者模式（observer）─────────────────────────────────────────────────────
//
// 「噬生・魔物觀測者」v3.12（2011 年的 Delphi 程式）在比對時不是一律精確 ——
// 它會在特定檔次上把取整邊界放寬一點點。這一段是那條規則的移植。
//
// 為什麼要放寬，看 fullRates 的閉合式就懂了：
//
//     fullRates[n] = 0.04n + 0.01*floor((n-1)/5) + (n > 0 && n % 5 === 0 ? 0.005 : 0)
//                              ~~~~~~~~~~~~~~~~     ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
//                              n % 5 === 1 跳一階    n % 5 === 0 多出半階
//
// 五個週期裡有兩個位置不規律。乘上 (lvl - 1) 之後，那半階會把 BP 推到剛好卡在
// 取整邊界上，Math.floor 就會差 1 —— 高等野寵「明明是這隻卻算不出來」多半是這樣來的。
// 原程式的作法是：**只在那兩個餘數上**，額外試一次 ±tol 之後再取整；
// 其餘餘數一律精確比對，能力值七項也一個容差都沒有。
//
// 所以這不是「模糊比對」，是針對成長係數表不規則處做的取整邊界修補。

/** 容差下限／上限。原程式裡是兩個 x87 80-bit Extended 常數。 */
const OBSERVER_TOL_MIN = 0.01;
const OBSERVER_TOL_MAX = 0.1;

/**
 * 容差 = clamp((當前等級 - 入手等級) / 200, 0.01, 0.1)。
 *
 * 等級跨越越多，成長誤差累積越大，容差就越寬。沒有入手等級時傳 0。
 */
function observerTolerance(lvl, catchLvl = 0) {
    const raw = (lvl - catchLvl) / 200;
    return Math.min(OBSERVER_TOL_MAX, Math.max(OBSERVER_TOL_MIN, raw));
}

/**
 * 某一軸的檔次要試哪些偏移量。第一個恆為 0（先試不偏移）。
 *
 * - `n % 5 === 0` 且 `n > 1` → `[0, -tol, +tol]`（多出半階，兩邊都可能偏）
 * - `n % 5 === 1`            → `[0, -tol]`（floor 剛跳一階，誤差單向）
 * - 其餘                      → `[0]`
 */
...
/** 五軸都不偏移時的寬放量 —— 精確比對用的，剪枝一格都不放寬。 */
const ZERO_SLACK = { stat: [0, 0, 0, 0, 0], sum: 0 };

/**
 * 剪枝時要多留多少寬放量。
 *
 * ⚠️ 偏移量加在 **BP** 上，傳到能力值會被 `M_BP` 放大（生命那一列的係數和是 17），
 * 所以拿 `tol` 本身當寬放量會把靠偏移才成立的解在進到比對之前就剪掉。
 * 逐軸算「這一列在可偏移的那幾軸上的係數絕對值和 × tol」才是真正的上界。
 */
function nudgeSlack(nudges, tol) {
    const movable = nudges.map(n => n.length > 1);
    return {
        stat: M_BP.map(row => tol * row.reduce(
            (acc, c, i) => movable[i] ? acc + Math.abs(c) : acc, 0)),
        // 總 BP 是五軸直接相加，每個可偏移的軸各貢獻 tol
        sum: tol * movable.filter(Boolean).length,
    };
}

/** 取整方式必須跟 BP.calcRealNum 一致，否則邊界會差 1。 */
function fixStat(n) {
...
function statsEqual(x0, x1, x2, x3, x4, tHp, tMp, tAtk, tDef, tAgi) {
    return (
        fixStat(8 * x0 + 2 * x1 + 3 * x2 + 3 * x3 + x4) === tHp &&
...
/**
 * 逐軸套上偏移量後再比對，任何一組吻合就算數。
 *
 * 偏移加在 **BP** 上（每軸一個），因為要修的誤差就是從 `fullRates[檔次[i]]`
 * 那一項來的，量綱對得上。回傳 `-1` 不吻合、`0` 不必偏移就吻合、`1` 靠偏移才吻合。
 *
 * 三軸以上同時落在不規則檔次才會超過 4 組組合，實務上多半只有 1～2 組。
 */
function observerMatch(nudges, x, tHp, tMp, tAtk, tDef, tAgi) {
    const total = nudges[0].length * nudges[1].length * nudges[2].length
        * nudges[3].length * nudges[4].length;
    for (let combo = 0; combo < total; combo++) {
        let rest = combo;
        let nudged = false;
        const v = [x[0], x[1], x[2], x[3], x[4]];
        for (let i = 0; i < 5; i++) {
            const k = rest % nudges[i].length;
            rest = (rest / nudges[i].length) | 0;
            if (k !== 0) {
                v[i] += nudges[i][k];
                nudged = true;
            }
...
function RealGuessRaw(Pts,input,opts) {
    const token = input.trim().split(/ /);
    const nums = token.slice(1).map(n => parseInt(n));
    // 輸入字串可能只給到 agi 或 notorderpoint，補滿 lvl..targetGrow 這 8 格，
    // opts 才會落在對的位置上（每一格的預設值都在 guess() 裡處理）。
    while (nums.length < 8) nums.push(undefined);
    return RealGuess(Pts,token[0], ...nums, opts);
}

/**
 * @param {object} [opts] 比對模式，見 GrowRange#guess。不填就是原本的精確比對。
 */
function RealGuess(Pts,name, lvl, hp, mp, attack, def, agi, notorderpoint,targetGrow,opts) {
    ...
    try {
        ...
        const results = rng.guess(stat,notorderpoint, targetGrow, opts);
...
export {observerTolerance, tierNudges, OBSERVER_TOL_MIN, OBSERVER_TOL_MAX};
